...
func NewHeaderBuilder(credentialService service.CredentialService, logger *zap.Logger) *HeaderBuilder {
...
func (hb *HeaderBuilder) BuildHeaders(
	...
	headerRules []*model.ProviderHeaderRule,
	providerID string,
) error {
...
func (hb *HeaderBuilder) resolveHeaderValue(
	...
	case "strategy":
		if val, ok := ctx.GetComputedValue(valueSource); ok {
			return fmt.Sprint(val), nil
		}
		return "", fmt.Errorf("computed value not found: %s", valueSource)

	case "param":
		if val, ok := ctx.InputParams[valueSource]; ok {
			return fmt.Sprint(val), nil
		}
